...
| `ui/typography.gd` | 書体と文字サイズ。**CJK フォントの同梱は必須**（13章 13.1） |
| `assets/fonts/` | 日本語フォント候補3種（SIL OFL 1.1） |
...
7. **数値の表示は `Fmt` を通す。** 直接 `str()` しない（05章 5.8）。
   毎フレーム変わる数値のラベルは `theme_type_variation = &"Counter"` か `&"Numeric"`
   にして等幅数字にする。しないと桁が左右に踊る。
